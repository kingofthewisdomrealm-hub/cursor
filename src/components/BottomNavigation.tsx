...
export function BottomNavigation({ currentView, onNavigate }: BottomNavigationProps) {
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-slate-900/95 backdrop-blur border-t border-amber-500/20 z-30">
          ...
          {NAV_ITEMS.map(({ id, label, icon: Icon }) => (
            <button
              ...
              className={`flex flex-col items-center gap-0 px-2 py-0.5 rounded-lg transition-colors ${
                currentView === id ? 'text-amber-400' : 'text-slate-500'
              }`}
            >
